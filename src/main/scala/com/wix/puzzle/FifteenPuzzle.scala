...
class FifteenPuzzle(dimension: Int = 4) {
  ...
  def init: PlayState = {
    val board = Random.shuffle((1 until dimension * dimension).toVector) ++ Vector(holeValue)
    PlayState(board, Point(dimension - 1, dimension - 1))
  }

  def up(playState: PlayState): PlayState = {
    if (playState.hole.y == 0) playState
    else switch(playState, playState.hole.copy(y = playState.hole.y - 1))
  }
  ...
  def left(playState: PlayState): PlayState = {
    ...
    else switch(playState, playState.hole.copy(x = playState.hole.x - 1))
  }
  ...
  private def switch(playState: PlayState, point: Point): PlayState = {
  ...
  def prettyPrint(gameState: PlayState): Unit = {
    ...
    gameState.board.zipWithIndex.foreach { case (value, idx) =>
      val displayValue = if (value == PlayState.holeValue) "_" else value.toString
